...
type WalletTrackerProps = {
  embedded?: boolean;
};

export function WalletTrackerContent({ embedded = false }: WalletTrackerProps) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const bounds = containerRef.current instanceof HTMLElement
        ? containerRef.current.getBoundingClientRect()
        : null;

      const baseWidth = embedded && bounds?.width ? bounds.width : window.innerWidth;

      const baseHeight = embedded && bounds?.height ? bounds.height : window.innerHeight;

      const targetWidth = Math.min(baseWidth, 1200);

      const targetHeight = Math.min(baseHeight * 0.68, embedded ? 360 : 620);
...
/* const glowGradient = ctx.createRadialGradient(centerX, centerY, globeRadius * 0.8, centerX, centerY, globeRadius * 1.3);
  ...
  return (
    ...
    >
      ...
      <canvas
        ref={canvasRef}
        className={`${embedded ? "absolute" : "fixed"} left-1/2 z-0 pointer-events-none -translate-x-1/2`}
        style={{
          top: embedded ? "150px" : "300px",
          width: "min(100%, 1100px)",
          height: embedded ? "320px" : "520px",
          background: "transparent",
          // Changed brightness from 0.88 to 0.5 to dim the whole element
      ...
      <div
        ...
        className={`absolute inset-x-0 top-0 z-1 ${embedded ? "h-36" : "h-60"}`}
...
`,
      ...
      {!embedded && <div className="relative z-10">
      ...
      <div className={`relative z-[2] text-white font-sans selection:bg-[#10b981]/30 selection:text-[#10b981] overflow-x-hidden ${
        embedded ? "min-h-full" : "min-h-screen"
      }`}>

        <div className={embedded ? "px-3 py-8 sm:px-5 sm:py-10" : "py-20 md:py-24 px-4 sm:px-6"}>

          <div className={`text-center ${embedded ? "space-y-5" : "space-y-10"}`}>


            <h1
              ref={titleRef}
              className={`font-bold tracking-tight leading-[1.1] text-balance drop-shadow-2xl ${
                embedded ? "text-3xl sm:text-4xl" : "text-4xl md:text-6xl lg:text-7xl"
              }`}
            >
            ...
            <p
              ref={subtitleRef}
              className={`text-white/50 font-medium max-w-2xl mx-auto ${
                embedded ? "text-sm sm:text-base" : "text-lg md:text-xl"
              }`}
            >
            ...
            <div ref={searchRef} className={`${embedded ? "max-w-lg" : "max-w-2xl"} mx-auto w-full group`}>
              ...
              <form onSubmit={handleSearchSubmit} className="relative">
                ...
                <div className="relative flex items-center bg-black/60 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl transition-all duration-300 group-focus-within:border-[#10b981]/50">
                  ...
                  <Search className={`${embedded ? "left-4 h-4 w-4" : "left-5 w-5 h-5"} absolute text-white/30`} />
                  ...
                  <input
                    ...
                    className={`w-full bg-transparent text-white placeholder-white/20 focus:outline-none ${
                      embedded ? "py-4 pl-11 pr-28 text-sm" : "px-14 py-6 text-lg"
                    }`}
                  ...
                  <button
                    ...
                    className={`absolute text-sm font-bold text-black bg-[#10b981] rounded-xl hover:bg-[#34d399] transition-colors shadow-[0_0_20px_rgba(16,185,129,0.3)] ${
                      embedded ? "right-2 px-4 py-2" : "right-4 px-6 py-2"
                    }`}
        ...
        <div ref={betaRef} className={`max-w-5xl mx-auto px-4 sm:px-6 ${embedded ? "mb-6" : "mb-20"}`}>

          <div className={`rounded-3xl backdrop-blur-xl bg-black/40 border border-white/5 transition-colors duration-500 ${
            embedded ? "p-4" : "p-8"
          }`}>

            <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">

              <div className={`${embedded ? "h-10 w-10" : "w-14 h-14"} flex-shrink-0 rounded-2xl flex items-center justify-center`}>

                <AlertTriangle className={`${embedded ? "h-8 w-8" : "w-12 h-12"} text-yellow-500`} />
              ...
              <div className="flex-1">
                ...
                <p className={`text-white/80 leading-relaxed ${embedded ? "text-sm" : "text-md md:text-base"}`}>

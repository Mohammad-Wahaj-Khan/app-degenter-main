...
const FEE_RECEIVER_ADDRESS =
  process.env.NEXT_PUBLIC_RECEIVER_WALLET_SWAP?.trim() || "";
const FEE_RATE_BPS = 1n; // 0.01%
const BPS_DENOMINATOR = 10_000n;
const MEMO = "Traded from degenter.io";

const splitSwapFee = (amountInMicro: string) => {
  const totalAmount = BigInt(amountInMicro);
  if (totalAmount <= 1n) {
    return { feeAmount: 0n, swapAmount: totalAmount };
  }

  let feeAmount = (totalAmount * FEE_RATE_BPS) / BPS_DENOMINATOR;
  if (feeAmount < 1n) feeAmount = 1n;
  if (totalAmount - feeAmount < 1n) feeAmount = totalAmount - 1n;

  return {
    feeAmount,
    swapAmount: totalAmount - feeAmount,
  };
};
...
export default function SwapInterface({
  ...
  const resolvedApiBase = apiBase ?? API_BASE_URL;
  ...
  const PAGE_TOKEN_INIT_IS_ZIG =
    isZigRef(tokenSymbol) ||
    isZigRef(
      PAGE_TOKEN_INIT.type === "native"
        ? (PAGE_TOKEN_INIT as any).denom
        : (PAGE_TOKEN_INIT as any).contract
    );
  ...
  const [direction, setDirection] = useState<"payToReceive" | "receiveToPay">(
    PAGE_TOKEN_INIT_IS_ZIG ? "receiveToPay" : "payToReceive"
  );
  ...
  const [other, setOther] = useState<SwapAsset>(
    PAGE_TOKEN_INIT_IS_ZIG ? { ...USDC } : { ...ZIG }
  );

  const defaultCounterAsset = useMemo<SwapAsset>(() => {
    const usdc = tokenList.find(
      (token) => token.symbol?.trim().toUpperCase() === "USDC"
    );
    if (!usdc) return { ...USDC };

    return isCw20Contract(usdc.denom)
      ? {
          type: "cw20",
          contract: usdc.denom.trim(),
          symbol: usdc.symbol,
          icon: usdc.imageUri || tokenIcon,
          decimals: usdc.exponent ?? 6,
        }
      : {
          type: "native",
          denom: cleanDenom(usdc.denom),
          symbol: usdc.symbol,
          icon: usdc.imageUri || tokenIcon,
          decimals: usdc.exponent ?? 6,
        };
  ...
  useEffect(() => {
    ...
    const pageTokenIsZig = isZigRef(PAGE_TOKEN.symbol) || isZigRef(pageRef);
    if (!pageTokenIsZig) return;

    const otherRef =
      other.type === "native" ? (other as any).denom : (other as any).contract;
    const counterRef =
      defaultCounterAsset.type === "native"
        ? (defaultCounterAsset as any).denom
        : (defaultCounterAsset as any).contract;
    const shouldApplyZigDefault =
      otherRef === pageRef || isZigRef(other.symbol) || isZigRef(otherRef);
    const shouldRefreshUsdcAsset =
      other.symbol?.trim().toUpperCase() === "USDC" && otherRef !== counterRef;
    if (!shouldApplyZigDefault && !shouldRefreshUsdcAsset) return;

    if (otherRef !== counterRef) setOther(defaultCounterAsset);
    if (shouldApplyZigDefault && direction !== "receiveToPay") {
      setDirection("receiveToPay");
    }
  }, [PAGE_TOKEN, defaultCounterAsset, direction, other]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`${resolvedApiBase}/tokens/swap-list`, {
          headers: API_HEADERS,
        });
  ...
  const runRouteFetch = useCallback(async () => {
    try {
      setErr("");
      const url = `${resolvedApiBase}/swap?from=${encodeURIComponent(
        fromRef
  ...
  useEffect(() => {
    ...
    const doSim = async () => {
      ...
      try {
        ...
        const { swapAmount } = splitSwapFee(amountInMicro);
        if (swapAmount <= 0n) {
          setSimQuoteOut(0);
          return;
        }
        const swapAmountMicro = swapAmount.toString();
  ...
  async function onSwap() {
    try {
      ...
      if (!FEE_RECEIVER_ADDRESS) {
        throw new Error("Swap fee receiver is not configured");
      }
      const { feeAmount, swapAmount } = splitSwapFee(amountInMicro);
      if (swapAmount <= 0n) throw new Error("Amount is too small after fees");
      const swapAmountMicro = swapAmount.toString();
      ...
      const buildNativeFeeMsg = (denom: string) =>
        feeAmount > 0n && FEE_RECEIVER_ADDRESS
          ? {
          ...
          : null;
      const buildCw20FeeMsg = (contract: string) =>
        feeAmount > 0n && FEE_RECEIVER_ADDRESS
          ? buildExecuteMsg(contract, {
              transfer: {
          ...
          : null;
      const signSwapWithFee = (feeMsg: any, swapMsg: any) =>
        client.signAndBroadcast(
          address,
          feeMsg ? [feeMsg, swapMsg] : [swapMsg],
          "auto",
          MEMO
        );
      ...
      if (routePairs.length === 1) {
        ...
        if (activePay.type === "native") {
          ...
          const payDenom = (activePay as any).denom;
          const msg = {
            swap: {
              max_spread: max_spread_str,
              offer_asset: {
                amount: swapAmountMicro,
                info: { native_token: { denom: payDenom } },
              },
              to: address,
            },
          };
          const funds = coins(swapAmountMicro, payDenom);
          const res = await signSwapWithFee(
            buildNativeFeeMsg(payDenom),
            buildExecuteMsg(pair.pairContract, msg, funds)
          );
          ...
          const payContract = (activePay as any).contract;
          const res = await signSwapWithFee(
            buildCw20FeeMsg(payContract),
            buildExecuteMsg(payContract, sendMsg)
          );
          setTxHash(res.transactionHash);
          setShowTxAlert(true);
          setAmountIn(""); // Clear the input field after successful swap
        ...
        if (activePay.type === "native") {
          ...
          const res = await signSwapWithFee(
            buildNativeFeeMsg(payDenom),
            buildExecuteMsg(ROUTER_CONTRACT, msgNative, funds)
          );
          setTxHash(res.transactionHash);
          setShowTxAlert(true);
          setAmountIn(""); // Clear the input field after successful swap
    ...
    <div className="my-3 bg-black/30 rounded-xl p-4 duration-200 border border-[#808080]/40">
      ...
      <div className="flex items-center justify-between my-3">
        <div className="flex items-center gap-3">
          <div className="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-black/50 px-3 py-1.5 text-sm text-white">

...
const swapRouteParam = (asset: SwapAsset) =>
  ...
  isZigRef(asset.type === "native" ? asset.denom : asset.contract)
    ...
    ? asset.denom
    : asset.contract;
const assetInfoFromRef = (ref: string) =>
  isCw20Contract(ref)
    ? { token: { contract_addr: ref } }
    : { native_token: { denom: isZigRef(ref) ? "uzig" : ref } };
const assetInfoFromAsset = (asset: SwapAsset) =>
  asset.type === "native"
    ? { native_token: { denom: asset.denom } }
    : { token: { contract_addr: asset.contract } };
...
export default function SwapInterface({
  ...
  const routeRefsForCurrentQuote = useCallback(() => {
    const quotedRefs =
      smartRouteQuote?.routeTokens
        ?.map((token) => token.denom?.trim())
        .filter(Boolean) ?? [];

    if (quotedRefs.length === routePairs.length + 1) {
      return quotedRefs;
    }

    const start =
      activePay.type === "native" ? activePay.denom : activePay.contract;
    const end =
      activeReceive.type === "native"
        ? activeReceive.denom
        : activeReceive.contract;

    if (routePairs.length <= 1) return [start, end];
    return [start, ...Array(routePairs.length - 1).fill("uzig"), end];
  }, [activePay, activeReceive, routePairs.length, smartRouteQuote]);
  ...
  const runRouteFetch = useCallback(async () => {
    try {
      ...
      if (pairs.length === 0) {
        setRoutePairs([]);
        setErr("No valid route found - pairs not registered in router");
      } else {
        setRoutePairs(pairs);
      }
  ...
  useEffect(() => {
    ...
    const doSim = async () => {
      ...
      try {
        ...
        const refs = routeRefsForCurrentQuote();
        let currentAmount = amountInMicro;

        for (let idx = 0; idx < routePairs.length; idx += 1) {
          const offerInfo =
            idx === 0 ? assetInfoFromAsset(activePay) : assetInfoFromRef(refs[idx]);
          const sim: any = await qClientRef.current.queryContractSmart(
            routePairs[idx].pairContract,
            {
              simulation: {
                offer_asset: {
          ...
          currentAmount = String(sim?.return_amount || "0");
        }

        const return2Micro = Number(currentAmount);
        const out =
          activeReceive.decimals === 0
            ? return2Micro
  ...
  async function onSwap() {
    try {
      ...
      if (routePairs.length === 1) {
        ...
        expectedOutMicro = currentAmount;
      ...
      if (routePairs.length === 1) {
        ...
        const operations = routePairs.map((p, idx) => {
          ...
          const offer_asset_info = isFirst
            ? assetInfoFromAsset(activePay)
            : assetInfoFromRef(refs[idx]);

          const ask_asset_info = isLast
            ? assetInfoFromAsset(activeReceive)
            : assetInfoFromRef(refs[idx + 1]);
